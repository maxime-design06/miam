...
export function SiteHeader() {
  ...
  return (
    <header className="flex items-center justify-between mb-10 gap-2">
      <Link href="/" className="font-display text-2xl sm:text-3xl text-papaya shrink-0">
        miam
      </Link>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-1.5 px-2.5 sm:px-3.5 py-1.5 rounded-full transition whitespace-nowrap ${
                isActive ? "bg-leaf text-white" : "text-foreground hover:bg-surface"
              }`}
            >
              <Icon className="w-4 h-4 sm:hidden" aria-hidden="true" />
              <span className="hidden sm:inline">{item.label}</span>
            </Link>
          );
        })}
        <Link href="/admin" className="ml-1 sm:ml-2 flex items-center shrink-0 text-foreground">
